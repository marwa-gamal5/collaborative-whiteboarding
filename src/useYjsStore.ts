import {
  ...
  export function useYjsStore({
	roomId = 'myroom/',
	hostUrl = import.meta.env.MODE === 'development' ? 'ws://localhost:1234' : '',
	shapeUtils = [],
	role = 'student', // Role parameter added (default to 'student')
  }: Partial<{
	hostUrl: string;
	roomId: string;
	version: number;
	shapeUtils: TLAnyShapeUtilConstructor[];
	role: 'teacher' | 'student'; // Define role types
  }>) {
	...
	useEffect(() => {
	  ...
	  function handleSync() {
		...
		// Only allow synchronization if the role is not 'student'
		if (role !== 'student') {
		...
		// Initialize the store with the yjs doc records or create initial records
	...
	}, [room, yDoc, store, yStore, meta, role]); // include role in dependency array
